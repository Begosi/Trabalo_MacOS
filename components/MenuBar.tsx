...
export function MenuBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 h-7 bg-[var(--mac-bg)]/80 backdrop-blur-md border-b border-white/10 flex items-center justify-between px-3 sm:px-4 z-50 text-[12px] sm:text-[13px] font-medium text-[var(--mac-text)]">
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <span className="font-bold cursor-default select-none">macOS</span>
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        <Search className="w-3.5 h-3.5 sm:w-4 sm:h-4 cursor-pointer hover:opacity-80" />
        <Wifi className="hidden xs:inline w-3.5 h-3.5 sm:w-4 sm:h-4 cursor-default" />
        <Battery className="w-3.5 h-3.5 sm:w-4 sm:h-4 cursor-default" />
        <span className="cursor-default sm:hidden select-none">{shortTime}</span>
        <span className="cursor-default hidden sm:inline select-none">{time}</span>
      </div>
    </div>
  );
}
