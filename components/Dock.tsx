...
export function Dock() {
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-50">
      <div 
        ref={dockRef}
        className="flex items-end gap-2 px-3 pb-2 pt-3 bg-white/10 backdrop-blur-xl border border-white/20 rounded-2xl shadow-2xl"
      >
        {dockItems.map((item) => (
          <div key={item.id} className="relative group flex flex-col items-center">
            {/* Tooltip */}
            <div className="absolute -top-10 opacity-0 group-hover:opacity-100 transition-opacity bg-black/70 text-white text-[11px] px-2 py-1 rounded-md pointer-events-none whitespace-nowrap">
              {item.label}
            </div>
            ...
            <button
              ...
              className="dock-icon w-12 h-12 flex items-center justify-center bg-gradient-to-b from-white/20 to-white/5 border border-white/20 rounded-xl hover:bg-white/30 transition-colors shadow-lg"
            >
              <div className="text-[var(--mac-text)] opacity-90">{item.icon}</div>
            </button>
            
            {/* Dot indicator */}
            <div className="w-1 h-1 bg-white/50 rounded-full mt-1.5 opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
        ))}
      </div>
    </div>
