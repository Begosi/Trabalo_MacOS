...
export default function Home() {
  ...
  return (
    <div className="flex flex-col gap-16 sm:gap-32 pb-40 overflow-x-hidden">
      {/* Hero Section */}
      <section id="hero" ref={heroRef} className="relative min-h-[85vh] sm:min-h-[90vh] flex items-center justify-center pt-10">
        ...
        <div className="z-10 w-full max-w-4xl px-4">
          <MacWindow title="Finder" className="hero-window max-w-3xl mx-auto shadow-[0_0_100px_rgba(0,113,227,0.2)]">
            <div className="flex flex-col items-center text-center py-10 px-4 sm:py-20 sm:px-8">
              <h1 className="hero-title text-3xl sm:text-5xl md:text-7xl font-bold tracking-tight mb-4 sm:mb-6 flex flex-wrap justify-center gap-x-2 sm:gap-x-3">
                {"A Experiência macOS".split(" ").map((w,i)=><span key={i} className="word inline-block">{w}</span>)}
              </h1>
              <p className="hero-subtitle text-sm sm:text-xl text-[var(--mac-muted)] max-w-xl mx-auto mb-6 sm:mb-10 leading-relaxed">
              ...
              <div className="hero-badges flex gap-3 sm:gap-4">
                <span className="px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-[var(--mac-blue)]/20 text-[var(--mac-blue)] font-medium text-xs sm:text-sm border border-[var(--mac-blue)]/30">
                  Darwin Core
                </span>
                <span className="px-3 py-1 sm:px-4 sm:py-1.5 rounded-full bg-white/10 text-[var(--mac-text)] font-medium text-xs sm:text-sm border border-white/10">
                  Aqua GUI
                </span>
              </div>
            </div>
      ...
      <div className="max-w-5xl mx-auto w-full px-3 sm:px-4 flex flex-col gap-16 sm:gap-32 relative z-10">
        <Section01 />
        <Section02 />
        <Section03 />
        <Section04 />
